...
folder_path = '/home/ray/2020-self-driving-car-project/Tracking/tracker_output/'
# lidar_data path (The directory until you see lots of log folders)
lidar_path = '/media/ray/arg2TB-Ray/argoverse/test/'
result_dirs = [d for d in os.listdir(
...
while not rospy.is_shutdown():
    for dirs in result_dirs:
        ...
        for filename in sorted(os.listdir(folder_path + dirs + '/per_sweep_annotations_amodal/')):
            resultsf = open(folder_path + dirs +
                            '/per_sweep_annotations_amodal/' + filename, 'r')
            ...
            for result in results:
                ...
                marker.header.stamp = rospy.Time.from_sec(
                    result['timestamp']/100000000.0)
                marker.ns = 'object'
                marker.id = count
                # The lifetime of the bounding-box, you can modify it according to the power of your machine.
                marker.lifetime = rospy.Duration(0.23)
                ...
                if result['track_label_uuid'] not in ids:  # for visualize tracking---
                    ...
                    marker1.text = str(
                        ids.index(result['track_label_uuid']))  # ----
            ...
            for lidar in sorted(os.listdir(lidar_path + dirs + '/lidar/')):
                if str(tstamp) in lidar:
                    ...
                    fields = [PointField('x', 0, PointField.FLOAT32, 1), PointField('y', 4, PointField.FLOAT32, 1), PointField(
                        'z', 8, PointField.FLOAT32, 1), PointField('intensity', 12, PointField.UINT32, 1), ]
                    lidar_pub.publish(pcl2.create_cloud(
                        header, fields, p[:, :4]))
            if markerArray:
                print(dirs + '/' + filename)
